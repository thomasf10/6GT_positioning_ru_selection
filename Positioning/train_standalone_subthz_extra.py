"""
Sweep variant of `train_standalone_subthz.py`.

Runs the same standalone sub-THz positioning training repeatedly, each time
...
ones (deterministic given `extra_train_seed`).
...
def main():
    ...
    sweep_dir = Path('stored_models_positioning_subthz_extra') / f'sweep_{sweep_timestamp}'
    ...
    # Pre-shuffle the extra candidates so nested-N runs share the same prefix.
