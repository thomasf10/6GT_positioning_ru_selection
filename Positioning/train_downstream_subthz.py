"""
Downstream training script for 2D positioning from sub-THz CSI.

Same Conv3D backbone + 2D regression head as `train_standalone_subthz.py`,
but the conv/proj/fc backbone is initialized from a checkpoint produced by
`RU_selection/train_subthz_csi_ru_beam_sel.py` and then fine-tuned on the
on-grid positioning dataset.
"""
import json
import sys
import time
...
def main():
    ...
    batch_size = 8
    epochs = 150
    lr = 1e-3                # smaller than from-scratch — typical for fine-tuning
    ...
    run_dir = Path('stored_models_positioning_subthz_downstream') / run_name
    ...
    # If the pretrained kernels don't match the current data antenna counts,
    # the loaded conv weights would have a different shape and the transfer
    # would fail. Catch this up front.
    ...
    # `num_aps` is just the input channel count of the first Conv3d; for
    # sub-THz it carries the number of RUs sending pilots.
    ...
    evaluate_position_cdf(
        ...
        title='Downstream (fine-tuned) sub-THz: test positioning error CDF',
        csi_key=CSI_KEY,
    )
