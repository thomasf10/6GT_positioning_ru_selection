"""
Sweep variant of `train_standalone_sub10.py`.

Runs the same standalone sub-10 GHz positioning training repeatedly, each time
with a different number of additional UEs (`n_extra_train_users`) drawn from
the saved `train_user_ids` list. Smaller sweep values are subsets of larger
ones (deterministic given `extra_train_seed`), so improvements vs. n_extra can
be attributed to data volume rather than user selection.

All runs of a sweep land in a single `sweep_{timestamp}/` directory, with a
`sweep_summary.csv` aggregating the test metrics across runs.
"""
import json
import sys
import time
...
def main():
    ...
    # --- Sweep: number of extra training UEs (None = full available) ---
    n_extra_sweep = [100, 500, 1000, None]

    # --- Model config (shared across the sweep) ---
    conv_channels = 16
    conv_layers = 3
    proj_channels = 16
    pooled_ue = 2
    ...
    sweep_dir = Path('stored_models_positioning_sub10_extra') / f'sweep_{sweep_timestamp}'
    ...
    # --- Build the on-grid set once (same for every sweep iteration) ---
    ...
    # Pre-shuffle the extra candidates so that nested-N runs share the same
    # prefix of UEs (smaller N is a subset of larger N).
    ...
    # ----------------------------------------------------------------- #
    # Single training run for a given n_extra
    # ----------------------------------------------------------------- #
    def run_one(n_extra_requested):
        ...
        run_name = f'xtra{n_extra_label}'
        run_dir = sweep_dir / run_name
        run_dir.mkdir(parents=True, exist_ok=True)

        print(f'\n{"=" * 70}\n=== Run: {run_name} ===\n{"=" * 70}')
        print(f'  on_grid={len(on_grid_ids)} + extras={len(extra_ids)} '
        ...
        cdf_stats = evaluate_position_cdf(
            ...
            title=f'Standalone sub-10 GHz (+{len(extra_ids)} extras): test positioning error CDF',
        )
    ...
    with open(sweep_csv, 'w', encoding='utf-8') as f:
        f.write(
            'n_extra_requested,n_extra_picked,train_size,val_size,test_size,'
    ...
    print(f'{"n_extra":<10}{"train":>8}{"mean[m]":>10}{"rmse[m]":>10}{"p50":>8}{"p90":>8}{"p95":>8}')
    for r in sweep_results:
        print(
            f'{r["n_extra_label"]:<10}{r["train_size"]:>8}'
