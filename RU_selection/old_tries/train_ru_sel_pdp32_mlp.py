...
class RUSelectionPDPMLP(nn.Module):
    """Simple MLP for RU selection from PDP-32 inputs."""

    def __init__(self, input_size, hidden_size=512, num_ru_ids=160, dropout=0.3):
        super().__init__()
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_size // 2, num_ru_ids),
        )

    def forward(self, sub10_pdp):
        x = sub10_pdp.reshape(sub10_pdp.shape[0], -1)
        return self.net(x)


def ru_id_loss(ru_id_logits, labels, criterion=None):
    """Cross-entropy loss on global RU ID (same behavior as existing script)."""
...
def main():
    ...
    results_root = Path('pdp32_results')
    ...
    input_size = int(np.prod(pdp_shape))
    sample_pdp_raw = sample_data['sub10_pdp']
    sample_pdp_norm = normalize_pdp_db(sample_pdp_raw)

    print('Sample shapes:')
    print(f'  sub10_pdp shape = {pdp_shape}, flattened input_size = {input_size}')
    print(
    ...
    # Class weights from TRAIN split only (same principle as train_ru_sel_only.py)
    ...
    run_name = 'pdp32_mlp_ru_only'
    writer = SummaryWriter(str(log_dir / run_name))
    best_ckpt_path = str(checkpoint_dir / 'best_model_pdp32_mlp_ru_only.pt')
    ...
    evaluate_nmse_cdf(
        ...
        save_path=str(plot_dir / 'nmse_cdf_test_pdp32_mlp.png'),
        title='NMSE CDF: Optimal vs MLP RU Selection (PDP32)',
    )
    ...
    plt.tight_layout()
    plot_path = str(plot_dir / 'training_plots_pdp32_mlp_ru_only.png')
    plt.savefig(plot_path)
    tikz_path = str(plot_dir / 'training_plots_pdp32_mlp_ru_only.tex')
